...
if __name__ == '__main__':
    ...
    individuals = session.query(Individual) \
        .filter(Individual.scenario_id == 115, Individual.generation == 54, Individual.individual_number == 0)
    ...
    CA_CONFIG.etc['test_patterns'] = [create_binary_pattern(alphabet=alphabet, length=N, r=uniform(0, 1)) for _ in range(100)]
    ...
    for iii, individual in enumerate(individuals):
        ...
        x = []
        points = 0
        patterns = CA_CONFIG.etc['test_patterns'][30:31]
        for i, pattern in enumerate(patterns):
            ...
            a, b = last[0], second_last[0]

            if a != b and all(x == a for x in last) and all(x == b for x in second_last):
                points += 1
                x.append('█')
                ...
                plt.figure()
                plt.title(f'Synchronization task\nInitial state: {W} white, {B} black')
                plt.imshow(
                    [
